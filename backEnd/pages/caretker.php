...
// Fetch tortoises
$tortoises = $conn->query("SELECT t.tortoise_id, t.name, s.common_name, t.estimated_age_years, t.sex, t.health_status, h.assessment_date as last_check FROM tortoises t JOIN species s ON t.species_id = s.species_id LEFT JOIN health_assessments h ON t.tortoise_id = h.tortoise_id GROUP BY t.tortoise_id ORDER BY t.tortoise_id LIMIT 5");
...
<head>
    ...
    <style>
        /* copy all styles from original caretaker.html – keep exactly */
...
<div class="dashboard">
    ...
    <div class="navbar"><div class="nav-links" id="navLinks"><span class="nav-item active" data-table="tortoises"><i class="fas fa-turtle"></i> Tortoise records</span><span class="nav-item" data-table="enclosures"><i class="fas fa-fence"></i> Enclosures</span><span class="nav-item" data-table="tasks"><i class="fas fa-clipboard-list"></i> Tasks</span><span class="nav-item" data-table="health"><i class="fas fa-notes-medical"></i> Health</span><span class="nav-item" data-table="environment"><i class="fas fa-temperature-low"></i> Environment</span><span class="nav-item" data-table="feeding"><i class="fas fa-utensils"></i> Feeding</span></div><div class="logout-btn">Logout</div></div>
    <div class="stats-cards"><div class="stat-card"><i class="fas fa-turtle"></i><div class="stat-info"><h3><?php echo $tortoises->num_rows; ?></h3><span>tortoises</span></div></div><div class="stat-card"><i class="fas fa-fence"></i><div class="stat-info"><h3><?php echo $enclosures->num_rows; ?></h3><span>enclosures</span></div></div><div class="stat-card"><i class="fas fa-tasks"></i><div class="stat-info"><h3><?php echo $tasks->num_rows; ?></h3><span>pending tasks</span></div></div><div class="stat-card"><i class="fas fa-heartbeat"></i><div class="stat-info"><h3><?php echo $healths->num_rows; ?></h3><span>health checks today</span></div></div></div>

    <!-- Tortoise Table -->
    <div class="table-wrapper" id="tortoises"><div class="table-header"><h2>Individual tortoise records</h2><div class="action-bar"><button class="btn btn-edit">Edit</button><button class="btn btn-remove">Remove</button><button class="btn btn-add">Add</button><button class="btn btn-search">Search</button></div></div>
    ...
    <!-- Enclosures Table -->
    <div class="table-wrapper" id="enclosures"><div class="table-header"><h2>Enclosure management</h2><div class="action-bar">...</div></div>
...
<script>
    // same tab switching JS as original
